...
class AlphaVantageAPI:
    ...
    DATA_COLUMN_NAME = {
        TickerInterval.DAILY: "Time Series (Daily)",
        TickerInterval.WEEKLY: "Weekly Time Series",
        TickerInterval.MONTHLY: "Monthly Time Series",
    }
    ...
    def fetch_function_data(self, function: str, json_data_name: str, **kwargs):
        ...
        if json_data_name in data:
            ...
            raise ValueError(
                f"Data for {json_data_name} not found in the response: {data}")
    ...
    def fetch_ticker_data(self, ticker: str, interval: TickerInterval = TickerInterval.MONTHLY):
        df = self.fetch_function_data(
            ...
            json_data_name=self.DATA_COLUMN_NAME[interval],
            symbol=ticker,
            outputsize="full"
        )
